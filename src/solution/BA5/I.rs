...
pub fn initialize_matrix(matrix: &mut Vec<Vec<Option<i32>>>) {
    let lenw = matrix.len();
    for i in 0..lenw {
        matrix[i][0] = Some(PENALTY*i as i32);
    }
}
...
pub fn fit_alignment(v:&String, w: &String, score: &mut Vec<Vec<Option<i32>>>, track: &mut Vec<Vec<Track>>) -> (usize,usize) {
    ...
    for iv in 1..=lenv {
        for iw in 1..=lenw {
            ...
            if (iv==lenv) && (score[iw][iv] > score[max_pos.1][max_pos.0]) {
                max_pos = (iv,iw);
            }
        }
    }
...
pub fn run(content:&Vec<String>) {
    ...
    let mut score_matrix: Vec<Vec<Option<i32>>> = vec![vec![Some(0);lenv+1];lenw+1];
